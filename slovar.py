...
for a in range(1, 100): # Перебор всех значений
    f = 1 # Программа долна понимать, что "f" должна в ней присутствовать, поэтому записываем её опять
    for x in range(100):
...
for i in range(len(n)-1):
    for j in range(i + 1, len(n)): # Как выяснилось, "j" дает чуть больше шансов на правильное выполнение
...
# 36037
srt = open('24.txt') # Пусть читает и собрает нам список "n"
n = srt.readline()
cnt = 0
mx = 0
for i in range(1, len(n)-4): # "-4" Потому что у нас должно быть 4 буквы сразу
    if n[i]=='X' or n[i]=='Y' or n[i]=='Z':
        cnt += 1
    if n[i:i+4]=='XZZY': # Не знаю что это
        cnt = 0
    mx = max(mx, cnt)
    print(mx, cnt)
    # Короче, это говно работает, но почему-то выдает не совсем тот ответ
