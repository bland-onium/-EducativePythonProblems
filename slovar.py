...
for srt in range(174457, 174505+1):
    ...
    if len(set(dev))==2:
        print(*dev)
# 26
'''В текстовом файле записан набор натуральных чисел, не превышающих 109. Гарантируется, что все числа различны. Необходимо определить,
сколько в наборе таких пар нечётных чисел, что их среднее арифметическое тоже присутствует в файле,
и чему равно наибольшее из средних арифметических таких пар'''
midmax = 0 #Максимальное из средних
numb = [] # Первый список
numb2 = set() # второй список (четные)
cnt = 0
srt = open('26 (3).txt')
n = int(srt.readline())
for i in range(n): # Выводит намчисла из всей последовательости
    x = int(srt.readline())
    if x % 2 == 1: numb.append(x) # Проверяем делимость элемента на 2 и добавляем в один из списков
    else: numb2.add(x)
numlen = set(numb) # Не знаю зачем, но без этого никуда
for i in range(len(numb)-1): # Задаем обычный алгоритм перебора
    for j in range(i+1,len(numb)):
        mid = (numb[i]+numb[j])//2 # Среднее ИЗ ПАРЫ
        if (mid in numlen) or (mid in numb2): # ПРоверяем наличие среднего в файле
            cnt += 1
            midmax = max(mid, midmax)
            print(cnt, midmax)
